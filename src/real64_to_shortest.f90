...
contains
    ...
    function d2shortest(fn) result(str)
        ...
        integer(kind=int64) :: mv, mp, mm
        integer(kind=int64) :: mmshift
        integer(kind=int64) :: dp, dv, dm
        ...
        if (ieee_exponent == 0) then
            ...
            e2 = ieee_exponent - DOUBLE_EXPONENT_BIAS - DOUBLE_MANTISSA_BITS - 2
        ...
        mv = 4*m2
        mp = 4*m2 + 2
        if (m2 /= shiftl(1_int64, DOUBLE_MANTISSA_BITS) .or. ieee_exponent <= 1) then
            mmShift = 1
        else
            mmShift = 0
        end if
        ...
        if (e2 >= 0) then
            ...
            if (q <= 21) then
                if (mod(mv, 5) == 0) then
                    dv_is_trailing_zeros = multipleOfPowerOf5(mv, q)
                else if (accept_upper_bound(even)) then
                    dm_is_trailing_zeros = multipleOfPowerOf5(mm, q)
                else if (multipleOfPowerOf5(mp, q)) then
                    dp = dp - 1
                end if
            end if
        else
            q = max(0, shiftr(-e2*732923, 20) - 1)
            i = -e2 - q
            ...
            if (q <= 1) then
                ...
                if (accept_upper_bound(even)) then
        ...
        if (scientific_notation) then
            do i = 0, olength - 2
                c = int(mod(output, 10), int32)
                output = output/10
                str(idx + olength - i:idx + olength - i) = char(48 + c)
            end do
            str(idx:idx) = char(48 + int(mod(output, 10), int32))
            str(idx + 1:idx + 1) = '.'
            idx = idx + olength + 1
            if (olength == 1) then
                str(idx:idx) = '0'
            ...
            if (expn >= 100) then
                str(idx:idx) = char(48 + int(expn/100, int32))
                idx = idx + 1
                expn = mod(expn, 100)
                str(idx:idx) = char(48 + int(expn/10, int32))
                idx = idx + 1
            else if (expn >= 10) then
                str(idx:idx) = char(48 + int(expn/10, int32))
                idx = idx + 1
            end if
            str(idx:idx) = char(48 + int(mod(expn, 10), int32))
        else
            if (expn < 0) then
                str(idx:idx) = '0'
                idx = idx + 1
                ...
                current = idx
                do i = 1, olength
                    str(current + olength - i:current + olength - i) = char(48 + int(mod(output, 10), int32))
                    output = output/10
                    idx = idx + 1
                end do
                idx = idx - 1
                ...
                do i = 1, olength
                    str(idx + olength - i:idx + olength - i) = char(48 + int(mod(output, 10), int32))
                    output = output/10
                end do
                idx = idx + olength
                do i = olength, expn
    ...
    pure function decimal_length(v) result(r)
        ...
        if (v >= 1000000000000000000_int64) then
            r = 19; return
        else if (v >= 100000000000000000_int64) then
            r = 18; return
        else if (v >= 10000000000000000_int64) then
            r = 17; return
        else if (v >= 1000000000000000_int64) then
            r = 16; return
        else if (v >= 100000000000000_int64) then
            r = 15; return
        else if (v >= 10000000000000_int64) then
            r = 14; return
        else if (v >= 1000000000000_int64) then
            r = 13; return
        else if (v >= 100000000000_int64) then
            r = 12; return
        else if (v >= 10000000000_int64) then
            r = 11; return
        else if (v >= 1000000000_int64) then
            r = 10; return
        else if (v >= 100000000_int64) then
            r = 9; return
        else if (v >= 10000000_int64) then
            r = 8; return
        else if (v >= 1000000_int64) then
            r = 7; return
        else if (v >= 100000_int64) then
            r = 6; return
        else if (v >= 10000_int64) then
            r = 5; return
        else if (v >= 1000_int64) then
            r = 4; return
        else if (v >= 100_int64) then
            r = 3; return
        else if (v >= 10_int64) then
            r = 2; return
        else
            r = 1; return
        end if
    ...
    pure function mulPow5divPow2(m, i, j) result(r)
        ...
        integer(kind=int64) :: mHigh, mLow
        integer(kind=int64) :: bits13, bits03, bits12, bits02, bits11, bits01, bits10, bits00
        integer(kind=int32) :: actual_shift

        mHigh = shiftr(m, 31)
        mLow = iand(m, 2147483647_int64)
        bits13 = mHigh*POW5_SPLIT(i*POW5_ARRAY_NCOL + 1)
        bits03 = mLow*POW5_SPLIT(i*POW5_ARRAY_NCOL + 1)
        bits12 = mHigh*POW5_SPLIT(i*POW5_ARRAY_NCOL + 2)
        bits02 = mLow*POW5_SPLIT(i*POW5_ARRAY_NCOL + 2)
        bits11 = mHigh*POW5_SPLIT(i*POW5_ARRAY_NCOL + 3)
        bits01 = mLow*POW5_SPLIT(i*POW5_ARRAY_NCOL + 3)
        bits10 = mHigh*POW5_SPLIT(i*POW5_ARRAY_NCOL + 4)
    ...
    pure function mulPow5InvDivPow2(m, i, j) result(r)
        ...
        mHigh = shiftr(m, 31)
        mLow = iand(m, 2147483647_int64)
        bits13 = mHigh*POW5_INV_SPLIT(i*POW5_ARRAY_NCOL + 1)
        bits03 = mLow*POW5_INV_SPLIT(i*POW5_ARRAY_NCOL + 1)
        bits12 = mHigh*POW5_INV_SPLIT(i*POW5_ARRAY_NCOL + 2)
        bits02 = mLow*POW5_INV_SPLIT(i*POW5_ARRAY_NCOL + 2)
        bits11 = mHigh*POW5_INV_SPLIT(i*POW5_ARRAY_NCOL + 3)
        bits01 = mLow*POW5_INV_SPLIT(i*POW5_ARRAY_NCOL + 3)
